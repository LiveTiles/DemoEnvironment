...
.NOTES
    ...
    v0.9
        First Release. Identified 2Do´s:
            ...
            - handle this for reinstalls: Approve-PnPTenantServicePrincipalPermissionRequest -RequestId $request.Id -Force -Connection $pnpSharePointAdminContext
                    Approve-PnPTenantServicePrincipalPermissionRequest : Permission entry already exists
...
#region Set Constants
    ...
    if ($outputMaxLength -le 84) {$outputMaxLength = 84}  #"Getting LiveTiles installation files and deploying packages to SharePoint"
    $waitVeryLong = 60
    $waitLong = 10
    $waitShort = 5
#endregion
...
#region Add Apps to Site Collection
    $output = Write-LiveTilesHost -messageType PROCESS -message ("Adding local apps to Hub SiteCollection" + $SharePointTenantAdmin)
...
#region Add webparts to frontpage
    $output = Write-LiveTilesHost -messageType PROCESS -message ("Adding webparts to Hub Frontpage" + $SharePointTenantAdmin)
